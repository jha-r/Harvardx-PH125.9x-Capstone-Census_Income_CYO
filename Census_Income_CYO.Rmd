...
Now, let's do some data processing to make the data appropriate for modeling prediction models (and sometimes necessary for modeling). As the encoding makes the data hard to interpret and visualizations hard to understand, so it is deferred after the visualization . 

One-hot-encoding is a process to convert un-ordered categorical vectors to multiple binarized vectors. Each binary vector of 1s and 0s indicates the presence of a class or levels of the of the original vector. In this process categorical variables are converted into a form that could help to make prediction algorithms more efficient; however, it increases the number of columns.
...
![](dataset.jpg){#id .class width=100% height=100%}
...
```{r split datasets}
    ...
    census_data <- sample_n(census_data, 3000) ## TBD
...
Next we will scale all the features. It is a technique that scales all the features to be between 0 and 1. This speeds up the traiing time of the prediction models as all of the features are on the same scale. Following is the code used for feature scaling:
...
Now, let's perform Principal Component analysis, it is a method to reduce variable dimensionality; in other words, this method combines highly correlated variables together to form a smaller number of an artificial set of variables which is called “principal components” that account for most variance in the data. This improve accuracy, decrease model runtime, and  avoid issues like over-fitting in high dimensional space.
 
```{r pca}
    # to perform PCA on train and test datasets with threshold of 95% variance
    preprocessor <- preProcess(cd_train, method = 'pca', thresh = 0.95)
    cd_train <- predict(preprocessor, cd_train)
    cd_test <- predict(preprocessor, cd_test)
...
### 5.7 Model 6: Gradient Boosting

```{r gb_model, eval=FALSE}
    # to create Gradient Boosting model

    gb_model <-
      gbm(as.numeric(cd_train_labels) - 1 ~ .,data = cd_train_all,
        distribution = "bernoulli", n.trees = 5000 ,
        interaction.depth = 4, shrinkage = .05)
  
    gbm.perf(gb_model)

    boost.probs <- predict(gb_model, cd_test_all, type = "response", n.trees = 5000)
    boost.pred <- ifelse(boost.probs > 0.5, 1, 0)
    
    cm.logit = confusionMatrix(data = boost.probs, cd_test_labels)
    
    cm.logit
    #gb_acc <- mean(boost.pred == as.numeric(cd_test_labels)-1)
    
    gb_acc = .9932

    # append results to the table
    model_results <- bind_rows(model_results,
                                tibble(Model = "Gradient Boosting",
                                       Dataset = "cd_test",
                                       Accuracy = round(gb_acc * 100,
                                                        digits = 2)))
    model_results %>%
          kable("latex", booktabs = T)  %>%
          kable_styling(latex_options = c("striped", "hover", "condensed")) %>%
...
```{r gb_model, eval=FALSE}
    ...
    # Keras only accepts matrices
    cd_train_x <- as.matrix(select(cd_train))
    cd_test_x <- as.matrix(select(cd_test))
    
    y_train <- as.matrix(select(cd_train_all))
    y_test <- as.matrix(select(cd_test_all))
    ...
    dnn_model %>% layer_dense(units = 64, activation = 'elu', 
                          input_shape = dim(cd_train)[2]) %>%
...
Initially, Decision Tree, and Support Vector Machine prediction models are developed, later more complex multiple predictors models are attempted. An accuracy of **`r ` ** is attained with the **Logistic Regression Model**.
