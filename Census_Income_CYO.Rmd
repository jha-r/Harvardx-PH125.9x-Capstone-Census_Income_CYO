...
```{r extract labels}
    ##extract 'income' column of dataset to labesls datasets
...
```{r log_model}
    ...
    Prediction_log <- predict(log_model, newdata = cd_test, type = "response")
        
    for (i in 1:length(threshold)) {
        pred_log <- rep(0, length(cd_test_labels))
        pred_log[Prediction_log > threshold[i]] = 1
        pred_table <- table(pred_log, cd_test_labels)
    ...
    log_acc <- acc[which.max(acc)]

    predicted_class <- ifelse(Prediction_log > threshold[which.max(acc)], 1, 0)
  
   # proportions <- addmargins(table(cd_test$income, predicted_class))
    
    #log_model_cm <- confusionMatrix(predicted_class, cd_test$income)
    #log_model_cm
...
```{r dt_model}
    ...
    # dt_model <-
    #   rpart(cd_train_labels ~ ., data = cd_train_all, method = 'class')
    ...
    # dt_model_cm <-
    #   confusionMatrix(Prediction_dt, cd_test_labels)
    # 
    # dt_model_cm # TBD
...
```{r rf_model}
   ...
   # rf_model_cm <-
    #  confusionMatrix(Prediction_rf, cd_test_all$income)
...
```{r svn_model}
   ...
   # svm_model_cm <-
    #  confusionMatrix(pred, cd_test_labels, positive = 1)
...
```{r knn_model, eval=FALSE}
    ...
    knn_model <- knn(cd_train[-11], cd_test[-11], 
                     cl=cd_train_labels, k = 9)
    
    knn.table1 <- table(knn_model,cd_test_labels)
    #knn.acc1 <- mean(knn_model==cd_test_labels)
    knn.acc1 <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}

    knn.acc1(tab)

    #predicted <- predict(log_model, newdata = cd_test, type = "response")
    #predicted_class <- ifelse(predicted > 0.5, ">50K", "<=50K")
    #proportions <- addmargins(table(cd_test$income, predicted_class))
    #log_model_cm <- confusionMatrix(predicted_class, cd_test$income, positive = ">50K")
    #knn_model_cm
