...
```{r split datasets}
    ...
    #census_data <- sample_n(census_data, 6000) ## TBD
...
```{r knn_model, eval=FALSE}
    ...
    ##extract 'income' column of dataset as it will be used as 'cl' argument in knn function.

    census_data_train_target <- census_data[index,11]

    census_data_test_target <- census_data[-index,11]
    
    
    dim(census_data_train)
     dim(census_data_test)
      dim(census_data_train_target)
       dim(census_data_test_target)
    
    
    knn_model <- knn(census_data_train[-11], census_data_test[-11], 
                     cl=census_data_train_target, k = 9)
    
    knn.table1 <- table(knn_model,census_data_test_target)
    #knn.acc1 <- mean(knn_model==census_data_test_target)
